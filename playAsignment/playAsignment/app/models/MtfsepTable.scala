package models
// AUTO-GENERATED Slick data model for table Mtfsep
trait MtfsepTable {
  ...
  /** Entity class storing rows of table Mtfsep
  ...
  case class MtfsepRow(storenumber: Option[String] = None, storename: Option[String] = None, hubbedstorecode: Option[String] = None, allwdls: Option[Int] = None, totalexclgst: Option[Double] = None, gst: Option[Double] = None, totalincgst: Option[Double] = None)
  /** GetResult implicit for fetching MtfsepRow objects using plain SQL queries */
  implicit def GetResultMtfsepRow(implicit e0: GR[Option[String]], e1: GR[Option[Int]], e2: GR[Option[Double]]): GR[MtfsepRow] = GR{
    prs => import prs._
    MtfsepRow.tupled((<<?[String], <<?[String], <<?[String], <<?[Int], <<?[Double], <<?[Double], <<?[Double]))
  }
  /** Table description of table mtfsep. Objects of this class serve as prototypes for rows in queries. */
  class Mtfsep(_tableTag: Tag) extends profile.api.Table[MtfsepRow](_tableTag, "mtfsep") {
    def * = (storenumber, storename, hubbedstorecode, allwdls, totalexclgst, gst, totalincgst) <> (MtfsepRow.tupled, MtfsepRow.unapply)
  ...
  /** Collection-like TableQuery object for table Mtfsep */
  lazy val Mtfsep = new TableQuery(tag => new Mtfsep(tag))
}
