package models
// AUTO-GENERATED Slick data model for table PaperlessMapping
trait PaperlessMappingTable {
  ...
  /** Entity class storing rows of table PaperlessMapping
   ...
   *  @param paperlessLabelDetailId Database column paperless_label_detail_id SqlType(int8), Default(None)
  ...
  class PaperlessMapping(_tableTag: Tag) extends profile.api.Table[PaperlessMappingRow](_tableTag, Some("ecommerce"), "paperless_mapping") {
    ...
    /** Database column paperless_label_detail_id SqlType(int8), Default(None) */
    val paperlessLabelDetailId: Rep[Option[Long]] = column[Option[Long]]("paperless_label_detail_id", O.Default(None))
    ...
    /** Foreign key referencing Consignments (database name paperless_mapping_external_consignmentid_fkey) */
    lazy val consignmentsFk = foreignKey("paperless_mapping_external_consignmentid_fkey", externalConsignmentid :: HNil, Consignments)(r => Rep.Some(r.id) :: HNil, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
    /** Foreign key referencing EcommerceCustomerReturnHubbedIdMapping (database name paperless_mapping_external_return_hubbed_id_fkey) */
    lazy val ecommerceCustomerReturnHubbedIdMappingFk = foreignKey("paperless_mapping_external_return_hubbed_id_fkey", returnHubbedId :: HNil, EcommerceCustomerReturnHubbedIdMapping)(r => Rep.Some(r.id) :: HNil, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  /** Collection-like TableQuery object for table PaperlessMapping */
  lazy val PaperlessMapping = new TableQuery(tag => new PaperlessMapping(tag))
}
