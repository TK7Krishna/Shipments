package models
// AUTO-GENERATED Slick data model for table Mtfmarch
trait MtfmarchTable {
  ...
  /** Entity class storing rows of table Mtfmarch
  ...
  case class MtfmarchRow(storenumber: Option[String] = None, storename: Option[String] = None, hubbedstorecode: Option[String] = None, allwdls: Option[Int] = None, totalexclgst: Option[Double] = None, gst: Option[Double] = None, totalincgst: Option[Double] = None)
  /** GetResult implicit for fetching MtfmarchRow objects using plain SQL queries */
  implicit def GetResultMtfmarchRow(implicit e0: GR[Option[String]], e1: GR[Option[Int]], e2: GR[Option[Double]]): GR[MtfmarchRow] = GR{
    prs => import prs._
    MtfmarchRow.tupled((<<?[String], <<?[String], <<?[String], <<?[Int], <<?[Double], <<?[Double], <<?[Double]))
  }
  /** Table description of table mtfmarch. Objects of this class serve as prototypes for rows in queries. */
  class Mtfmarch(_tableTag: Tag) extends profile.api.Table[MtfmarchRow](_tableTag, "mtfmarch") {
    def * = (storenumber, storename, hubbedstorecode, allwdls, totalexclgst, gst, totalincgst) <> (MtfmarchRow.tupled, MtfmarchRow.unapply)
  ...
  /** Collection-like TableQuery object for table Mtfmarch */
  lazy val Mtfmarch = new TableQuery(tag => new Mtfmarch(tag))
}
