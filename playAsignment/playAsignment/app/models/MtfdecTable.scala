package models
// AUTO-GENERATED Slick data model for table Mtfdec
trait MtfdecTable {
  ...
  /** Entity class storing rows of table Mtfdec
  ...
  case class MtfdecRow(storenumber: Option[String] = None, storename: Option[String] = None, hubbedstorecode: Option[String] = None, allwdls: Option[Int] = None, totalexclgst: Option[Double] = None, gst: Option[Double] = None, totalincgst: Option[Double] = None)
  /** GetResult implicit for fetching MtfdecRow objects using plain SQL queries */
  implicit def GetResultMtfdecRow(implicit e0: GR[Option[String]], e1: GR[Option[Int]], e2: GR[Option[Double]]): GR[MtfdecRow] = GR{
    prs => import prs._
    MtfdecRow.tupled((<<?[String], <<?[String], <<?[String], <<?[Int], <<?[Double], <<?[Double], <<?[Double]))
  }
  /** Table description of table mtfdec. Objects of this class serve as prototypes for rows in queries. */
  class Mtfdec(_tableTag: Tag) extends profile.api.Table[MtfdecRow](_tableTag, "mtfdec") {
    def * = (storenumber, storename, hubbedstorecode, allwdls, totalexclgst, gst, totalincgst) <> (MtfdecRow.tupled, MtfdecRow.unapply)
  ...
  /** Collection-like TableQuery object for table Mtfdec */
  lazy val Mtfdec = new TableQuery(tag => new Mtfdec(tag))
}
